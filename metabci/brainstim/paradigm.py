...
import numpy as np
from math import pi
from psychopy import data, visual, event, core, prefs
...
class MI(VisualStim):
    ...
    def __init__(self, win, colorSpace="rgb", allowGUI=True):
        ...
        # 加载动态序列
        self.tex_left_anim = self._load_animation_sequence("left_hand_anim")
        self.tex_right_anim = self._load_animation_sequence("right_hand_anim")

        # 动画控制变量
        self.anim_index = 0
        self.anim_speed = 0.1  # 每秒切换多少帧
        self.anim_timer = 0

    def _load_animation_sequence(self, base_name):
        """加载动画序列图片"""
        anim_frames = []
        anim_dir = os.path.join(
            os.path.abspath(os.path.dirname(os.path.abspath(__file__))),
            "textures" + os.sep + "animations"
        )

        # 加载序列图片 (anim_0.png, anim_1.png, ...)
        frame_index = 0
        while True:
            frame_path = os.path.join(
                anim_dir,
                f"{base_name}_{frame_index}.png"
            )
            if os.path.exists(frame_path):
                anim_frames.append(frame_path)
                frame_index += 1
            else:
                break
        ...
        # 确保至少有一帧
        if not anim_frames:
            static_path = os.path.join(anim_dir, f"{base_name}.png")
            if os.path.exists(static_path):
                anim_frames.append(static_path)
            else:
                # 回退到静态图片
                if "left" in base_name:
                    anim_frames.append(self.tex_left)
                else:
                    anim_frames.append(self.tex_right)
    ...
    def config_color(
        ...
        """
        self.anim_speed = anim_speed    # 设置动画速度
        self.n_Elements = n_Elements
        self.stim_length = stim_length
        self.stim_width = stim_width
        ...
        self.image_left_stimuli = visual.ElementArrayStim(
            self.win,
            units="pix",
            elementTex=self.tex_left_anim[0],  # 初始帧
            elementMask=None,
            texRes=2,
            nElements=n_Elements,
            sizes=[[stim_length, stim_width]],
        ...
        self.image_right_stimuli = visual.ElementArrayStim(
            self.win,
            units="pix",
            elementTex=self.tex_right_anim[0],  # 初始帧
            elementMask=None,
            texRes=2,
            nElements=n_Elements,
            sizes=[[stim_length, stim_width]],
    ...
    def update_animation(self, dt):
        """更新动画帧"""
        self.anim_timer += dt
        frame_duration = 1.0 / (len(self.tex_left_anim) * self.anim_speed)

        if self.anim_timer >= frame_duration:
            self.anim_index = (self.anim_index + 1) % len(self.tex_left_anim)
            self.anim_timer = 0

            # 更新纹理
            self.image_left_stimuli.elementTex = self.tex_left_anim[self.anim_index]
            self.image_right_stimuli.elementTex = self.tex_right_anim[self.anim_index]
...
def paradigm(
    ...
    win,
    bg_color,
    hand_mode="both",  # 新增：选择实验模式 ("left", "right"或者"both")
    display_time=1.0,
    index_time=1.0,
    rest_time=0.5,
    response_time=2,
    ...
    if pdim == "ssvep":
        ...
        # 根据设置选择不同侧的实验
        if hand_mode == "left":
            conditions = [{"id": 1, "name": "left_hand"}]
        elif hand_mode == "right":
            conditions = [{"id": 0, "name": "right_hand"}]
        else:  # both 或其他情况
            conditions = [
                {"id": 1, "name": "left_hand"},
                {"id": 0, "name": "right_hand"},
            ]
        # 重复实验nrep次（试次处理）
        trials = data.TrialHandler(
            conditions,
            nrep,
            name="experiment",
            method="random" if len(conditions) > 1 else "sequential"
        )
        ...
        # 阶段1：初始刺激界面
        iframe = 0
        while iframe < int(fps * display_time) and not global_exit:
            ...
            VSObject.normal_right_stimuli.draw()    #分别绘制左右手掌
            iframe += 1
            win.flip()  #刷新窗口

        # 阶段2：开始刺激周期
        # 如果存在外部端口，则发送初始信号0
        if port and not global_exit:
            port.setData(0)
        # 遍历所有试次
        for trial in trials:
            # 设置退出按键q
            if global_exit:  # 检查全局退出标志
                break
            keys = event.getKeys(["q"])
            if "q" in keys:
                break
            ...
            # 阶段2-1：休息间隔
            if rest_time != 0 and not global_exit:
                VSObject.win.callOnFlip(port.setData, 1)
                iframe = 0
                while iframe < int(fps * rest_time) and not global_exit:
                    keys = event.getKeys(['q'])
                    if keys:
                        global_exit = True
                        break
            ...
            # 阶段2-2：提示阶段
            iframe = 0
            while iframe < int(fps * index_time) and not global_exit:
                keys = event.getKeys(['q'])
                if keys:
                    global_exit = True
                    break
            ...
            # 阶段2-3: 运动想象任务阶段
            iframe = 0
            while iframe < int(fps * image_time) and not global_exit:
                ...
                # 更新动画
                VSObject.update_animation(dt)

                if iframe == 0 and port and online:
                    VSObject.win.callOnFlip(port.setData, id + 2)
                elif iframe == 0 and port:
                    VSObject.win.callOnFlip(port.setData, id + 2)
                if iframe == port_frame and port:
                    port.setData(0)
            ...
            if inlet:
                ...
                iframe = 0
                while iframe < int(fps * response_time) and not global_exit:
